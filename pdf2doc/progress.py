# -*- coding: utf-8 -*-
# Ref: https://github.com/pypa/pipenv/blob/master/pipenv/progress.py
...
class Bar(object):
    ...
    def show(self, progress, count=None):
        ...
        if (time.time() - self.etadelta) > ETA_INTERVAL:
            ...
            self.eta = (
                sum(self.ittimes) / float(len(self.ittimes)) * (self.expected_size - progress)
            )
            self.etadisp = self.format_time(self.eta)
...
if __name__ == "__main__":
    list_bar = bar([i for i in range(10000)], width=32, label="test ")
    for i in list_bar:
        pass
